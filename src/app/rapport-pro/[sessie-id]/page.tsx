...
const VELD_ICONEN: Record<string, string> = {
  ...
  vervoer: "🚌", fotos: "📸", recensies: "⭐", hostProfiel: "👤", huisregels: "📋",
};
    ...
    <div className={`card p-5 sm:p-6 border ${bgKleur} space-y-4`}>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h3 className="font-display text-xl text-primary flex items-center gap-2">
          <span>{icoon}</span><span>{naam}</span>
        </h3>
        <VeldScore score={score} />
      </div>

      {veld.analyse && (
        <p className="italic text-text-secondary leading-relaxed">{veld.analyse}</p>
      )}
          ...
          <ul className="space-y-1">
            {veld.verbeterpunten.map((punt, i) => (
              <li key={i} className="text-sm text-text-secondary flex gap-2">
                <span className={`shrink-0 font-bold ${scoreKleur(score)}`}>→</span>
                <span>{punt}</span>
              </li>
            ))}
          </ul>
      ...
      {veld.herschrevenVersie && (
        <div>
          <p className="font-semibold text-primary text-sm mb-2">Herschreven versie</p>
          <div className="bg-surface rounded-xl p-4 relative group">
            <p className="text-primary text-sm whitespace-pre-wrap pr-10">{veld.herschrevenVersie}</p>
            <div className="absolute top-3 right-3">
              <CopyButton tekst={veld.herschrevenVersie} />
            </div>
          </div>
          {veld.herschrevenUitleg && (
            <p className="text-xs text-text-secondary mt-2 italic">{veld.herschrevenUitleg}</p>
          )}
        </div>
      )}
      ...
      {veld.herschrevenVersies && veld.herschrevenVersies.length > 0 && (
        <div>
          <p className="font-semibold text-primary text-sm mb-2">Herschreven versies</p>
          <div className="space-y-3">
            {veld.herschrevenVersies.map((v, i) => (
              <div key={i} className="bg-surface rounded-xl p-4">
                <div className="flex items-start justify-between gap-3 mb-1">
                  <p className="font-semibold text-primary text-sm">{v.versie}</p>
      ...
      {veld.aanbevelingen && veld.aanbevelingen.length > 0 && (
        <div>
          <p className="font-semibold text-primary text-sm mb-2">Aanbevelingen</p>
          <ul className="space-y-1">
            {veld.aanbevelingen.map((a, i) => (
              <li key={i} className="text-sm text-text-secondary flex gap-2">
                <span className="text-accent shrink-0">+</span><span>{a}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      {veld.voorbeeldReacties && veld.voorbeeldReacties.length > 0 && (
        <div>
          <p className="font-semibold text-primary text-sm mb-2">Voorbeeldreacties op reviews</p>
          <div className="space-y-3">
            {veld.voorbeeldReacties.map((r, i) => (
              <div key={i} className="bg-surface rounded-xl p-4 space-y-2">
                ...
                <div className="flex items-start justify-between gap-2">
                  <p className="text-sm text-primary">{r.aanbevolenReactie}</p>
        ...
        <div>
          ...
          <ul className="space-y-1">
            {veld.ontbrekendeVoorzieningen.map((item, i) => (
              <li key={i} className="text-sm text-text-secondary flex gap-2">
                <span className="text-warning">⚠️</span><span>{item}</span>
              </li>
            ))}
          </ul>
        </div>
...
export default function RapportProPagina() {
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center px-4">
        <div className="card p-8 max-w-md w-full text-center space-y-6">
          ...
          <a href="/starten-pro" className="btn-primary inline-block">
            Nieuwe Pro analyse starten
          </a>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-background">
      {deelOpen && <DeelModal onSluit={() => setDeelOpen(false)} overrideUrl={rapportPubliekUrl} titel="Listing Optimizer Pro Rapport" />}
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10 space-y-8">
        ...
        <div className="card p-6 space-y-4">
          ...
          {toonEmailInvoer && emailStatus !== "verstuurd" ? (
            ...
            >
              {emailVersturen ? "Versturen..." : emailStatus === "verstuurd" ? "✅ Verstuurd!" : emailStatus === "fout" ? "❌ Probeer opnieuw" : "📧 Rapport per email ontvangen"}
            </button>
          )}
        </div>
        ...
        {rapport.openingszin && (
          <div className="card p-6 flex gap-4 items-start">
            ...
            <p className="italic text-primary leading-relaxed text-lg">"{rapport.openingszin}"</p>
          </div>
        )}
        ...
        <div className="space-y-4">
          ...
          {velden.directBoeken && (
            <div className="card p-5 sm:p-6 border border-border space-y-3">
              <h3 className="font-display text-xl text-primary flex items-center gap-2"><span>⚡</span><span>Direct boeken</span></h3>
              {velden.directBoeken.huidigeInstelling && <p className="text-sm text-text-secondary">Huidige instelling: <span className="font-semibold text-primary">{velden.directBoeken.huidigeInstelling}</span></p>}
              {velden.directBoeken.aanbeveling && <p className="text-sm text-primary">{velden.directBoeken.aanbeveling}</p>}
              {velden.directBoeken.uitleg && <p className="text-sm text-text-secondary italic">{velden.directBoeken.uitleg}</p>}
            </div>
          )}

          {velden.annuleringsbeleid && (
            <div className="card p-5 sm:p-6 border border-border space-y-3">
              <h3 className="font-display text-xl text-primary flex items-center gap-2"><span>📅</span><span>Annuleringsbeleid</span></h3>
              {velden.annuleringsbeleid.huidigeInstelling && <p className="text-sm text-text-secondary">Huidige instelling: <span className="font-semibold text-primary">{velden.annuleringsbeleid.huidigeInstelling}</span></p>}
              {velden.annuleringsbeleid.aanbeveling && <p className="text-sm text-primary">{velden.annuleringsbeleid.aanbeveling}</p>}
              {velden.annuleringsbeleid.uitleg && <p className="text-sm text-text-secondary italic">{velden.annuleringsbeleid.uitleg}</p>}
            </div>
          )}
        </div>
            ...
            <div className="grid sm:grid-cols-3 gap-4">
              {rapport.actieplan.vandaag?.length > 0 && (
                <div className="card p-5 border-l-4 border-danger space-y-3">
                  ...
                  <ul className="space-y-2">{rapport.actieplan.vandaag.map((item, i) => (<li key={i} className="text-sm text-primary flex gap-2"><span className="text-danger shrink-0">•</span><span>{item}</span></li>))}</ul>
                </div>
              )}
              {rapport.actieplan.dezeWeek?.length > 0 && (
                <div className="card p-5 border-l-4 border-warning space-y-3">
                  ...
                  <ul className="space-y-2">{rapport.actieplan.dezeWeek.map((item, i) => (<li key={i} className="text-sm text-primary flex gap-2"><span className="text-warning shrink-0">•</span><span>{item}</span></li>))}</ul>
                </div>
              )}
              {rapport.actieplan.dezeMaand?.length > 0 && (
                <div className="card p-5 border-l-4 border-success space-y-3">
                  ...
                  <ul className="space-y-2">{rapport.actieplan.dezeMaand.map((item, i) => (<li key={i} className="text-sm text-primary flex gap-2"><span className="text-success shrink-0">•</span><span>{item}</span></li>))}</ul>
                </div>
              )}
            </div>
          </div>
        ...
        {rapport.bonusTips?.length > 0 && (
          <div className="card p-6 border border-accent/20 space-y-3">
            <h2 className="font-display text-2xl text-primary flex items-center gap-2"><span>🎁</span><span>Bonustips van Boni</span></h2>
            <ul className="space-y-2">{rapport.bonusTips.map((tip, i) => (<li key={i} className="text-sm text-primary flex gap-2"><span className="shrink-0">✨</span><span>{tip}</span></li>))}</ul>
          </div>
        )}
        ...
        <div className="card p-8 text-center space-y-4 bg-primary text-white border-0">
          <h2 className="font-display text-2xl">Wil je nog een advertentie analyseren?</h2>
          <p className="text-white/70 text-sm">Boni staat klaar om je volgende listing automatisch in te lezen en te analyseren.</p>
          <a href="/starten-pro" className="btn-primary inline-block w-full sm:w-auto text-center">
            Nieuwe Pro analyse starten →
          </a>
        </div>
      </div>
