...
export async function POST(request: Request) {
  try {
    ...
    // foto_bewerkingen aanmaken
    await admin.from("foto_bewerkingen").insert(
      fotos.map((f: { volgnummer: number }) => ({
        sessie_id: sessieId,
    ...
    const uploadTokens = await Promise.all(
      fotos.map(async (f: { volgnummer: number; type: string }) => {
        const ext = extFromType(f.type);
        const pad = `${sessieId}/${String(f.volgnummer).padStart(3, "0")}.${ext}`;
        const { data, error } = await admin.storage
          .from("foto-originelen")
          .createSignedUploadUrl(pad);
        if (error || !data) throw new Error("Upload URL genereren mislukt");
        return { volgnummer: f.volgnummer, pad, token: data.token };
      })
    );
