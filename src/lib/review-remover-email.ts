...
export function buildReviewRemoverEmailHtml(input: {
  ...
  const safeBaseUrl = input.baseUrl.startsWith("https://") ? input.baseUrl : "https://www.hostboni.com";

  const stappenHtml = input.stappenplan
    .map((stap, i) => `<li style="margin-bottom:8px;color:#374151;font-size:14px;">${i + 1}. ${escapeHtml(stap)}</li>`)
    .join("");
  ...
  return `
    <div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px;">
      ...
      <p style="color:#374151;">Hey ${escapeHtml(input.naam)}! Hier is de beoordeling van jouw recensie:</p>
      ...
      <div style="background:#f9fafb;border-radius:8px;padding:16px;margin:16px 0;">
        <p style="margin:0;font-size:14px;color:#1B2B4B;">${escapeHtml(input.onderbouwing)}</p>
      </div>
      <h2 style="font-size:16px;color:#1B2B4B;">Jouw bezwaarbrief</h2>
      <div style="background:#eef7fe;border-radius:8px;padding:16px;margin-bottom:16px;white-space:pre-wrap;font-size:14px;color:#1B2B4B;">${escapeHtml(input.bezwaarbrief)}</div>
      <h2 style="font-size:16px;color:#1B2B4B;">Stappenplan</h2>
      <ol style="padding-left:20px;">${stappenHtml}</ol>
      <div style="text-align:center;margin:32px 0;">
        <a href="${safeBaseUrl}/review-remover" style="background:#FF6B6B;color:white;padding:14px 32px;border-radius:10px;text-decoration:none;font-weight:bold;font-size:15px;">
          Nog een recensie checken →
        </a>
      </div>
      <p style="color:#9ca3af;font-size:12px;text-align:center;margin-top:24px;">
        Host Boni · <a href="${safeBaseUrl}" style="color:#9ca3af;">hostboni.com</a>
      </p>
    </div>`;
}
